...
export class AppointmentService {
  ...
  async createAppointment(appointmentData: AppointmentRequest): Promise<AppointmentResponse> {
    try {
      ...
      // Optional: n8n kann parallel in Supabase den Termin in Tabelle appointments schreiben
      // Wir geben hier nur die Daten zurück; UI kümmert sich um Statusanpassung
      return payload
    } catch (error) {
      return {
        success: false,
