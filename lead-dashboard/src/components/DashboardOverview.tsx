...
export function DashboardOverview({ onOpenLead }: Props) {
  ...
  // Mini-Karte: Nächster Schritt je Lead (Top 6)
  ...
  return (
    <div className="space-y-4">
      <KpiBar />
      {/* Mini-Karte: Nächster Schritt je Lead */}
      <div className="bg-white rounded-lg border p-3">
        <div className="text-sm font-semibold mb-2">Nächster Schritt je Lead</div>
        {nextSteps.length === 0 ? (
          <div className="text-sm text-gray-500">Keine nächsten Schritte gefunden</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {nextSteps.map(item => (
              <button key={item.leadId} className="text-left p-2 border rounded hover:bg-gray-50"
                      onClick={() => onOpenLead?.(item.leadId)}>
                <div className="text-gray-900 text-sm font-medium">{item.leadName}</div>
                <div className="text-xs text-gray-600">{item.next}{item.due ? ` • ${new Date(item.due).toLocaleDateString('de-DE')}` : ''}</div>
              </button>
            ))}
          </div>
        )}
