...
export function LeadForm({ lead, onSave, onCancel, isSubmitting = false }: LeadFormProps) {
  ...
  const form = useForm<LeadFormData>({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (tenantId) {
          const attemptNum = (Number(values.not_reached_count) || 0) + (values.phone_status === 'erreichbar' ? 0 : 0)
          await supabase
            .from('contact_attempts')
            .insert({
              tenant_id: tenantId as any,
              lead_id: lead.id,
              attempt_number: Math.max(1, attemptNum),
              reached: values.phone_status === 'erreichbar',
              mailbox_left: values.voicemail_left,
              phone_off: values.phone_switched_off,
              notes: values.quick_notes || values.documentation || null,
              next_attempt_date: values.phone_status === 'nicht_erreichbar' ? (updateData.follow_up_date as any) : null
            })
        }
      } catch {}
    }
  ...
  const getNextActionOptions = () => {
    if (form.values.phone_status === 'erreichbar') {
      return [
        { value: 'appointment', label: 'Termin' },
        { value: 'offer', label: 'Angebot' }
      ]
    } else if (form.values.phone_status === 'nicht_erreichbar' || form.values.phone_status === 'zurueckrufen') {
      return [
        { value: 'follow_up', label: 'Wiedervorlage' }
      ]
    } else if (form.values.phone_status === 'termin_vereinbart') {
      return [
  ...
  const getAttemptColor = (count: number) => {
  ...
  return (
    <form onSubmit={form.handleSubmit} className="space-y-6">
      ...
      {/* Telefonversuch */}
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Telefonversuch</h3>

        {/* Erstauswahl: erreicht / nicht erreicht */}
        <div className="flex items-center gap-2 mb-4">
          <button
            type="button"
            onClick={() => {
              form.setValue('phone_status', 'erreichbar')
            }}
            className={`px-3 py-1 rounded text-sm border ${form.values.phone_status === 'erreichbar' ? 'bg-green-100 border-green-300 text-green-800' : 'bg-white border-gray-300 text-gray-700'}`}
            disabled={isSubmitting}
          >
            ✅ Erreicht
          </button>
          ...
          {/* farbiger Counter-Button */}
          <button
            type="button"
            onClick={() => form.setValue('not_reached_count', Math.min(3, (Number(form.values.not_reached_count)||0) + 1))}
            className={`ml-auto px-3 py-1 rounded text-sm border ${getAttemptColor(Number(form.values.not_reached_count)||0)} border-gray-300`}
            title="Zähler erhöhen (max. 3)"
            disabled={isSubmitting}
          >
            {Math.min(3, Number(form.values.not_reached_count)||0)}x
          </button>
        </div>

        {/* Optionen nur sichtbar bei erreicht */}
        {form.values.phone_status === 'erreichbar' && (
          <div className="mt-4">
            <h4 className="text-md font-medium text-gray-900 mb-3">Aktion bei erreicht</h4>
        ...
        {/* Optionen nur sichtbar bei nicht erreicht */}
        {form.values.phone_status === 'nicht_erreichbar' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Checkbox
              checked={form.values.voicemail_left}
              onChange={(checked) => form.setValue('voicemail_left', checked)}
              label="Mobilbox besprochen"
              disabled={isSubmitting}
            />
            <Checkbox
              checked={form.values.phone_switched_off}
              onChange={(checked) => form.setValue('phone_switched_off', checked)}
              label="Telefon ausgeschaltet"
              disabled={isSubmitting}
            />
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Nicht erreicht Zähler</label>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => form.setValue('not_reached_count', Math.max(0, (Number(form.values.not_reached_count)||0) - 1))}
                  className="px-2 py-1 rounded border bg-white text-gray-700 border-gray-300"
                  disabled={isSubmitting}
                >
                  −
                </button>
                <span className={`px-3 py-1 rounded border ${getAttemptColor(Number(form.values.not_reached_count)||0)} border-gray-300`}>{Math.min(3, Number(form.values.not_reached_count)||0)}x</span>
                <button
                  type="button"
                  onClick={() => form.setValue('not_reached_count', Math.min(3, (Number(form.values.not_reached_count)||0) + 1))}
                  className="px-2 py-1 rounded border bg-white text-gray-700 border-gray-300"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
                ...
                onChange={(value) => {
                 ...
                 // Auto-Inkrement bei Nicht erreicht
                 if (value === 'nicht_erreichbar') {
                   const current = Number(form.values.not_reached_count) || 0
                   form.setValue('not_reached_count', current + 1)
                 }
               }}
      ...
      {/* PV-Kurzinfos & Notizen */}
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">PV-Kurzinfos & Notizen</h3>
        
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField label="PV kWp">
              <input
                type="number"
                step="0.1"
                value={form.values.pv_kwp}
                onChange={(e) => form.setValue('pv_kwp', e.target.value)}
                disabled={isSubmitting}
            ...
            <FormField label="Speicher kWh">
              <input
                type="number"
                step="0.1"
                value={form.values.storage_kwh}
                onChange={(e) => form.setValue('storage_kwh', e.target.value)}
                disabled={isSubmitting}
                className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
            ...
            <div className="grid grid-cols-1 gap-3">
              <Checkbox
                checked={form.values.has_backup}
                onChange={(checked) => form.setValue('has_backup', checked)}
                label="Notstrom"
                disabled={isSubmitting}
              />
              <Checkbox
                checked={form.values.has_ev_charger}
                onChange={(checked) => form.setValue('has_ev_charger', checked)}
                label="Ladestation"
                disabled={isSubmitting}
              />
              <Checkbox
                checked={form.values.has_heating_mgmt}
                onChange={(checked) => form.setValue('has_heating_mgmt', checked)}
                label="Heizungsmanagement"
                disabled={isSubmitting}
              />
            </div>
          </div>

          <FormField label="Kurz-Notizen">
            <textarea
              value={form.values.quick_notes}
              onChange={(e) => form.setValue('quick_notes', e.target.value)}
              disabled={isSubmitting}
              rows={3}
              className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50 disabled:cursor-not-allowed"
              placeholder="Wichtigste Punkte in 1-2 Sätzen"
            />
          </FormField>
        </div>
      </div>
