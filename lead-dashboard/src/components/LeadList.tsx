...
export function LeadList({ onLeadClick }: LeadListProps) {
  ...
  useEffect(() => {
    function onDocClick() { setStatusMenuFor(null); setMenuPos(null) }
    function onEsc(e: KeyboardEvent) { if (e.key === 'Escape') { setStatusMenuFor(null); setMenuPos(null) } }
    if (statusMenuFor) {
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="relative inline-flex items-center gap-2">
                      ...
                      <IconButton
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setStatusMenuFor(prev => prev === lead.id ? null : lead.id)
                          const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
                          setMenuPos({ top: rect.bottom + window.scrollY, left: rect.left + window.scrollX, width: Math.max(180, rect.width + 140) })
                        }}
                        title="Aktuellen Status ändern"
                      />

                      {statusMenuFor === lead.id && menuPos && createPortal(
                        (
                          <div
                            className="z-50 bg-white border rounded shadow text-xs"
                          ...
                          >
                            {LEAD_STATUS_OPTIONS.map(opt => (
                              <button
                                key={opt.value}
                                type="button"
                                className={`w-full text-left px-3 py-2 hover:bg-gray-100 ${lead.lead_status === opt.value ? 'bg-blue-50 text-blue-700' : 'text-gray-800'}`}
                                onClick={async () => {
                                  setStatusMenuFor(null)
                                  setMenuPos(null)
                                  if (opt.value !== lead.lead_status) {
                                    await updateLead({ id: lead.id, lead_status: opt.value as LeadStatus })
                                    await refetch()
                                  }
                                }}
                              >
